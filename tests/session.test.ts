...
describe('SessionKeyManager', () => {
    ...
    describe('Session Creation', () => {
        it('should create a session with valid configuration', async () => {
            ...
            expect(session.walletAddress).toMatch(/^0x[a-fA-F0-9]{40}$/);
        });

        it('should keep multiple active sessions for the same master key', async () => {
            const masterKey = {
                credentialId: 'test-credential-123',
                publicKeyX: BigInt('0x' + '1'.repeat(64)),
                publicKeyY: BigInt('0x' + '2'.repeat(64)),
            ...
            const firstSession = await manager.createSession(masterKey, {
                dailyLimitUSD: 100,
                perTransactionLimitUSD: 25,
            ...
            const secondSession = await manager.createSession(masterKey, {
                dailyLimitUSD: 200,
            ...
            const sessions = await manager.getSessionsForMasterKey(masterKey.keyHash);

            expect(sessions.map((session) => session.keyHash)).toEqual(
