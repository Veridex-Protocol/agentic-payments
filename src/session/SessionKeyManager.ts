...
export class SessionKeyManager {
  ...
  checkLimits(session: StoredSession, amountUSD: number): LimitCheckResult {
    // First, check if session is paused
    if (this.isSessionPaused(session)) {
      return {
        allowed: false,
        reason: `Session is paused${session.metadata.pauseReason ? `: ${session.metadata.pauseReason}` : ''}`,
   ...
   */
  isSessionPaused(session: StoredSession): boolean {
    return session.metadata.pausedAt != null;
  }

  /**
   * Pause a session. The session remains valid but cannot execute transactions.
   * Unlike revocation, pause is reversible.
   *
   * @param keyHash - Session key hash to pause
   * @param reason - Optional reason for pausing
   */
  async pauseSession(keyHash: string, reason?: string): Promise<boolean> {
    const session = await this.storage.getSession(keyHash);
    if (!session) return false;
    if (session.metadata.pausedAt != null) return false; // Already paused

    session.metadata.pausedAt = Date.now();
    if (reason) {
  ...
  /**
   * Resume a paused session. Clears the pause state and allows transactions again.
   *
   * @param keyHash - Session key hash to resume
   */
  async resumeSession(keyHash: string): Promise<boolean> {
    const session = await this.storage.getSession(keyHash);
    if (!session) return false;
    if (session.metadata.pausedAt == null) return false; // Not paused

    session.metadata.pausedAt = undefined;
    session.metadata.pauseReason = undefined;
    await this.storage.saveSession(session);
    return true;
  }
