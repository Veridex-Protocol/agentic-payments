...
export type WebhookChannelType = 'slack' | 'pagerduty' | 'http';
export type AlertSeverity = 'INFO' | 'WARNING' | 'CRITICAL';

export interface WebhookChannel {
    type: WebhookChannelType;
    url: string;
    /** Only dispatch alerts at or above this severity (default: all) */
    minSeverity?: AlertSeverity;
    /** Custom headers to include in the request */
    headers?: Record<string, string>;
    /** PagerDuty routing key (required for pagerduty type) */
    routingKey?: string;
...
export interface AlertConfig {
    ...
    /** @deprecated Use `webhooks` array instead */
    webhookUrl?: string;
    /** Multi-channel webhook dispatch with severity routing */
...
export class AlertManager {
    ...
    /**
     * Send webhook notification to a specific channel.
     */
    private async sendToChannel(channel: WebhookChannel, alert: SpendingAlert): Promise<boolean> {
        if (channel.enabled === false) return false;

        const severity = alert.type as AlertSeverity;
        if (channel.minSeverity && !this.meetsMinSeverity(severity, channel.minSeverity)) {
            return false;
        }

        try {
            const { url, headers, body } = this.formatForChannel(channel, alert);
            ...
            console.error(`[AlertManager] Webhook delivery to ${channel.type} failed:`, error);
            return false;
        }
    }

    /**
     * Format alert payload for the target channel type.
     */
                        ...
                        blocks: [
                            {
                                ...
                                text: {
                                    type: 'mrkdwn',
                                    text: `*${alert.type}*: ${alert.message}\n` +
                                        `Session: \`${alert.sessionKeyHash}\`\n` +
                                        `Spent: $${alert.dailySpentUSD.toFixed(2)} / $${alert.dailyLimitUSD.toFixed(2)}`,
                                },
                            },
                        ],
                    },
            ...
            case 'pagerduty':
                return {
                    ...
                    body: {
                        ...
                        event_action: alert.type === 'CRITICAL' ? 'trigger' : 'acknowledge',
                        payload: {
                            summary: alert.message,
                            severity: alert.type === 'CRITICAL' ? 'critical' : 'warning',
                            source: 'veridex-agent-sdk',
                            custom_details: {
                                sessionKeyHash: alert.sessionKeyHash,
                                dailySpentUSD: alert.dailySpentUSD,
    ...
    private meetsMinSeverity(actual: AlertSeverity, min: AlertSeverity): boolean {
        const levels: Record<AlertSeverity, number> = { INFO: 0, WARNING: 1, CRITICAL: 2 };
        return (levels[actual] ?? 0) >= (levels[min] ?? 0);
    }

    /**
     * Send webhook to legacy single URL (backward compat).
     */
    ...
    private notify(alert: SpendingAlert) {
        ...
        // Dispatch to multi-channel webhooks
        if (this.config.webhooks?.length) {
            for (const channel of this.config.webhooks) {
                this.sendToChannel(channel, alert).catch(() => {});
            }
        }
