...
export const PAUSE_SESSION_SCHEMA = {
  type: 'object',
  properties: {
    sessionKeyHash: { type: 'string', description: 'Session key hash to pause. If omitted, pauses the current session.' },
    reason: { type: 'string', description: 'Reason for pausing the session' }
  }
};

export const RESUME_SESSION_SCHEMA = {
  type: 'object',
  properties: {
    sessionKeyHash: { type: 'string', description: 'Session key hash to resume. If omitted, resumes the current session.' }
  }
};
